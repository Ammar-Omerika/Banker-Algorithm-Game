...
function generateProcesses() {
  ...
  for (let i = 1; i <= 5; i++) {
    processes.push({
      id: i,
      max: {
        A: getRandom(0, resources.A - 2),
        B: getRandom(0, resources.B - 2),
        C: getRandom(0, resources.C - 2),
      },
      alloc: { A: 0, B: 0, C: 0 },
      need: { A: 0, B: 0, C: 0 },
    });
...
function checkForDeadlock() {
  ...
  if (!canExecute) {
    ...
    document.getElementById("message").textContent = "Deadlock detected! Awarding 300 points.";
    return true;
  }
...
function renderGame() {
  ...
  document.getElementById("level").textContent = `Easy Level: ${currentLevel}`;
  document.getElementById("score").textContent = score;
